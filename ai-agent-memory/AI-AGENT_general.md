...
- `general/`目录除`Agents.md`外的内容会复制至`ai-agent-memory/`中作为常驻指令；默认仅需阅读`ai-agent-memory/`下除`criterion-*`系列目录之外的文档，遇到需要参考特定规范守则时，再按需查阅对应的`criterion-*`文档。
...
- 在项目级`Agents.md`中记录新增约定、规范调整与重要决策；若`general/user-temp-opinion.md`有新的选项或模板调整，应在此阶段同步确认。
