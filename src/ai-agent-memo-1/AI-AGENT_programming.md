...
本文件针对编程类任务提供在`AI-AGENT_general.md`基础上的额外要求。执行任何开发、测试、调试行为前，请确认已阅读通用指引并同步更新`AI-AGENT_working-status.csv`。
...
- **文档归档**：`AI-AGENT_working-status.csv`、`undetermined.md`、`optimized-task.md`、`mcp-log.csv`与`mcp-log.md`等辅助文档统一存放于`ai-agent-output/YYYY-MM-DDTHH-MM-SS_UTC+8_任务简述/`目录，任务简述需控制在8-20字。
...
- 注意，需要在每个流程图的第一行`@startuml`后，插入一行下面的语句以确保字符可正常输出
  ```
...
- 每实现一个功能单元或完成一次重要重构，都应：
  - 更新`AI-AGENT_working-status.csv`，标注当前子环节与进度。
...
- 同步更新以下内容（视任务而定）：
  ...
  - 运行脚本、配置样例、环境准备说明。
- 更新工作状态后，应先维护`CHANGELOG.md`，确认条目与影响描述完整；随后必须根据当次变更创建新的版本号段落（写入日期并重建空的`[Unreleased]`区块），再运行`git add .`并创建提交。
- 在项目根`Agents.md`或本文件记录新增约定、最佳实践、已知限制。
...
- 若任务涉及重要决策、约束或反复出现的风险，需在`Agents.md`或相关设计文档中记录原因与影响。
