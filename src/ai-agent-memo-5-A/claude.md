...
!!! 在开始任何工作前，必须先阅读本文件，并查阅`ai-agent-memory/`目录中的所有指引。

## 快速指引

- `ai-agent-memory/AI-AGENT_preparation.md`：任务前置准备的完整流程规范。
- `ai-agent-memory/AI-AGENT_step-before.md`：前置准备的详细步骤说明。
- `ai-agent-memory/undetermined-template.md`：待定项记录的格式模板。
- `ai-agent-memory/now-task.md`：待用户填写的任务描述文件。
